...
GLuint shader_planet;
mesh planet;
mesh_drawable planet_visual;
float planetRadius;
...
void initialize_data()
{
	...
	planetRadius = 1.0f;
	planet = createIcoSphere(planetRadius, 100);
	planet_visual = mesh_drawable(planet);
	updatePlanetMesh(planet, planetRadius, parameters);
	updatePlanetVisual(planet, planet_visual);
}
...
void display_interface()
{
	...
	update |= ImGui::SliderFloat("Influence", &parameters.influence, 0.1f, 5.0f);
