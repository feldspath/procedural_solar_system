...
namespace vcl {
	...
	void initialiseFrameBuffer(GLuint& framebuffer) {
		glGenFramebuffers(1, &framebuffer);
	}

	void attachTextureToFramebuffer(GLuint framebuffer, GLuint& textureColorbuffer, GLuint& rbo, unsigned int width, unsigned int height) {
	...
	void renderToFrameBuffer(GLuint framebuffer) {
		glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
		glEnable(GL_DEPTH_TEST);
	}

	void renderColorbuffer(GLuint textureColorbuffer, GLuint rbo, camera_around_center& camera, GLuint shader = quadDefaultShader) {
